...
class CalculateORPRequest(BaseModel):
    word: str = Field(
        ..., min_length=1, max_length=100, description="Word to calculate ORP for"
    )
...
class ProcessingStats(BaseModel):
    ...
    processed_count: int = Field(
        ge=0, description="Processed word count (with duplicates/pauses)"
    )
    estimated_time_300wpm: float = Field(
        ge=0, description="Estimated reading time at 300 WPM (seconds)"
    )
    estimated_time_500wpm: float = Field(
        ge=0, description="Estimated reading time at 500 WPM (seconds)"
    )
