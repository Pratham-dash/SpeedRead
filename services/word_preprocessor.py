...
class WordPreprocessor:
    ...
    def preprocess_with_headings(
        self, words_with_meta: List[Tuple[str, dict]]
    ) -> List[dict]:
        """
        ...
        for i, (word, meta) in enumerate(words_with_meta):
            ...
            if is_heading and not in_heading:
                ...
                for _ in range(self.pause_count):
                    processed.append(
                        {"word": " ", "is_heading": False, "display_multiplier": 1}
                    )
                in_heading = True
                heading_words = []
    ...
    def get_statistics(
        self, original_words: List[str], processed_words: List[str]
    ) -> dict:
        ...
        return {
            ...
            "expansion_ratio": (
                len(processed_words) / len(original_words) if original_words else 1.0
            ),
        }
    ...
    def customize_duplication(
        self, words: List[str], duplication_map: dict
    ) -> List[str]:
        processed = []
