...
class TextProcessor:
    ...
    def split_words_with_metadata(self, text: str) -> List[Tuple[str, dict]]:
        ...
        for line_idx, line in enumerate(lines):
            ...
            is_heading = (
                line_is_heading[line_idx] if line_idx < len(line_is_heading) else False
            )
        ...
        while word_idx < len(all_words):
            words_with_meta.append(
                (all_words[word_idx], {"is_heading": False, "is_all_caps": False})
            )
            word_idx += 1
    ...
    def _split_multi_hyphenated_words(self, words: List[str]) -> List[str]:
        ...
        for word in words:
            ...
            has_multiple_hyphens = (
                first_hyphen != -1 and word.find("-", first_hyphen + 1) != -1
            )
